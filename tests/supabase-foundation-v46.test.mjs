...
test("pins Supabase dependencies and protects the NAGA proxy", async () => {
  ...
  assert.match(edgeFunction, /modelNames/);
...
test("generates browser runtime config without accepting server secrets", async () => {
  ...
  assert.match(workflow, /prepare-cloudflare-entry\.mjs --output-dir public --mode ci/);
  assert.doesNotMatch(workflow, /run: npm run runtime:config/);
...
test("supports private collection spaces and owner-reviewed access requests", async () => {
  ...
  assert.match(html, /const APP_VERSION = 303/);
  ...
  assert.match(navigation, /data-menu-view="today"[^>]*><span class="menu-nav-icon"[^>]*>▶<\/span><span>学ぶ<\/span>/);
...
test("scopes answer-history hydration to the logged-in user and current collection", async () => {
  ...
  assert.match(html, /const ANSWER_HISTORY_LIMIT_V135 = 5000/);
  assert.match(html, /loadMyAttemptsForCollection\(collectionSlug, ANSWER_HISTORY_LIMIT_V135\)/);
  assert.match(html, /remoteHistorySyncScopeV135/);
