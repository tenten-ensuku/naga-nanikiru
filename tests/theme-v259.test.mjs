...
test('accessible My Page controls reflect current theme and early restoration precedes app markup', () => {
  ...
  assert.ok(html.indexOf('<script src="theme-v259.js?v=259"></script>') < html.indexOf('<style'));
  assert.match(html, /window\.MinkiruThemeV259\?\.bind\(\)/);
  assert.ok(html.indexOf('theme-v259.css?v=259') > html.indexOf('question-toolbar-v255.css'));
});
