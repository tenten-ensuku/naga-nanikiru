...
test('accessible My Page controls reflect current theme and early restoration precedes app markup', () => {
  ...
  assert.ok(html.indexOf('<script src="theme-v259.js?v=285"></script>') < html.indexOf('<style'));
  assert.match(html, /window\.MinkiruThemeV259\?\.bind\(\)/);
  assert.ok(html.indexOf('theme-v259.css?v=285') > html.indexOf('question-toolbar-v255.css'));
});
