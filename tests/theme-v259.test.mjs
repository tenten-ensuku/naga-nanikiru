...
test('My Page retains PC layout settings without duplicating the header theme control', () => {
  ...
  assert.ok(html.indexOf('<script src="theme-v259.js?v=308"></script>') < html.indexOf('<style'));
  assert.match(html, /id="themeQuickV295"/);
  assert.ok(html.indexOf('theme-v259.css?v=308') > html.indexOf('question-toolbar-v255.css'));
});
