...
test('My Page retains PC layout settings without duplicating the header theme control', () => {
  ...
  assert.ok(html.indexOf('<script src="theme-v259.js?v=309"></script>') < html.indexOf('<style'));
  assert.match(html, /id="themeQuickV295"/);
  assert.ok(html.indexOf('theme-v259.css?v=309') > html.indexOf('question-toolbar-v255.css'));
});
