...
test('V240 adds only a local presentation layer and preserves approved assets',()=>{
  ...
  assert.match(html,/workspace-ui-v240\.css\?v=241/); assert.match(html,/workspace-ui-v240\.js\?v=241/);
...
test('settings keep authority gates, named categories, transfer host and escaped profile text',()=>{
  ...
  for(const key of ['account','display','reactions','transfer']) assert.match(result,new RegExp(`data-settings-group-v240="${key}"`));
  assert.doesNotMatch(result,/<img>|data-settings-group-v240="teaching"/);
  assert.match(result,/data-settings-transfer-v240/);assert.match(result,/accountAuthButtonV240/);
