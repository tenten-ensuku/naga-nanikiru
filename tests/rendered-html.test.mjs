...
test("wires v65 learning UX, generator, Supabase bridge, metadata, and social image", async () => {
  const [index, page, layout] = await Promise.all([
    ...
    access(new URL("../public/og-v266.png", import.meta.url)),
  ]);
  assert.match(index, /const APP_VERSION = 299/);
  ...
  assert.match(index, /return filtered\.sort\(\(a, b\) => menuQuestionSortKeyV99\(a\) - menuQuestionSortKeyV99\(b\)\)/);
  ...
  assert.match(index, /getElementById\("menuButton"\)\.addEventListener\("click", returnFromQuestionV256\)/);
  ...
  assert.match(index, /おすすめ学習/);
  assert.match(index, /NAGA URLから追加/);
  assert.match(index, /property="og:image" content="https:\/\/minkiru\.naga-study\.workers\.dev\/og-v266\.png"/);
  ...
  assert.match(layout, /og-v266\.png/);
});
