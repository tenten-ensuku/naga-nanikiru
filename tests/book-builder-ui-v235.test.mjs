...
test('V287 renamed books show their saved title, including the legacy default collection',()=>{
  const source=html.slice(html.indexOf('function collectionDisplayNameV101('),html.indexOf('function collectionChooserTitleV186('));
  const display=vm.runInNewContext(source+'; collectionDisplayNameV101',{DEFAULT_COLLECTION_SLUG:'legacy-default'});
  assert.equal(display({share_slug:'legacy-default',title:'新しい問題集名'}),'新しい問題集名');
  assert.equal(display({share_slug:'another',title:'くにたそ チーム検討用'}),'くにたそ チーム検討用');
});
