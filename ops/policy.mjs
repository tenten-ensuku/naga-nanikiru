...
export function evaluate(state,metrics,sources,now=Date.now()) {
  // A reporting-only deployment must never arm, latch or resume application controls.
  if(state.readOnly===true)return {mode:'read-only',observeUntil:null,blocked:false,reasons:[],canResume:false};
...
export function buildMetrics(raw,egress,now=Date.now(),readOnly=false) {
  const metadataTtl=readOnly?48*HOUR:2*HOUR;
  ...
  for(const [id,label] of [['minkiru','みん切るDB'],['ranking','ランキングDB']]) {
    ...
    const status=!data?'unknown':result.failures||now-Date.parse(result.lastSuccessAt)>metadataTtl?'stale':'ok';
  ...
  const stale=['minkiru','ranking'].some(id=>raw[id]?.failures||now-Date.parse(raw[id]?.lastSuccessAt||0)>metadataTtl);
  const storageRows=[];
  ...
  for(const [id,label,limit,field] of [['workers-requests','Workersの本日リクエスト',100000,'workersRequests'],['r2-class-a','R2 Class A・今月',1e6,'classA'],['r2-class-b','R2 Class B・今月',1e7,'classB']]){
    ...
    metrics.push({id,label,kind:'requests',provider:'Cloudflare',unit:'requests',used:values?.[field]??null,limit,status:!values?'unknown':cf.failures||!periodMatches||now-Date.parse(cf.lastSuccessAt)>(readOnly?48*HOUR:HOUR)?'stale':'ok',observedAt:cf?.lastSuccessAt||null,source:'Cloudflare GraphQL Analytics',note:'WorkersはUTC日次、R2操作数はUTC暦月。アカウント合計。期間切替の未取得値は前回値として表示。',parts:[],details:[]});
  }
  const valid=egress&&Date.parse(egress.periodStart)<=now&&Date.parse(egress.periodEnd)>now;
  for(const [id,label,field] of [['egress','Supabase通常Egress','uncachedBytes'],['cached-egress','Supabase Cached Egress','cachedBytes']])metrics.push({id,label,provider:'Supabase',kind:'egress',used:valid?egress[field]:null,limit:5e9,unit:'bytes',status:valid?(now-Date.parse(egress.confirmedAt)>(readOnly?7*DAY:DAY)?'stale':'manual'):'unknown',observedAt:egress?.confirmedAt||null,source:'本人がDashboardで確認した期間合計',note:valid?egress.periodStart+' ～ '+egress.periodEnd+'（確認時点の値。自動取得ではありません）':'確認値未登録／期間外。API回数からの推定はしません。',parts:[],details:[]});
  return metrics;
}
