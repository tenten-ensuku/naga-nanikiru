-- V222: 既存の埋め込みコメントへ、現在のプロフィールに紐づく
-- Discordアイコンを再付与する。
-- コメント本文・添付画像・日時は変更せず、アイコンの表示情報だけを補完する。
    ...
    select p.avatar_url
     ...
     where p.avatar_url ~* '^https://(cdn\.discordapp\.com|media\.discordapp\.net|images-ext-[0-9]+\.discordapp\.net)/'
       and (
         ...
         or lower(btrim(comment.value->>'author')) in ('marlboro', 'マルモロ')
            and lower(btrim(p.display_name)) = 'marlboro0908'
         or lower(btrim(comment.value->>'author')) = '垣崎にま'
            and lower(btrim(p.display_name)) = 'kakisakinima'
         or lower(btrim(comment.value->>'author')) = 'くにたそ'
            and lower(btrim(p.display_name)) = 'kunimusya'
         or (
