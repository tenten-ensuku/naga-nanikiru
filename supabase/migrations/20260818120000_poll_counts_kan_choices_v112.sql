-- Count kan answers as a dedicated community-poll choice.
-- Existing boolean call answers remain compatible with call:yes/call:no.
  ...
  with all_attempts as (
    ...
    select
      case
        ...
        when lower(coalesce(answer ->> 'callDecision', '')) in ('kan', 'call:kan') then 'call:kan'
        when lower(coalesce(answer ->> 'callDecision', '')) in ('true', 'call:yes') then 'call:yes'
        when lower(coalesce(answer ->> 'callDecision', '')) in ('false', 'call:no') then 'call:no'
        else 'unknown'
      end as choice,
      count(*)::bigint as n
    from all_attempts
