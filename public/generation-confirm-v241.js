(function (root) {
  ...
  root.NagaGenerationConfirmV241 = Object.freeze({
    ask(document, destination, count) {
      ...
      if (!dialog || !description || dialog.open || !Number.isSafeInteger(count) || count < 1) return Promise.resolve(false);
      description.textContent = `保存先「${destination.label}」へ${count}問を追加します。${destination.kind === 'shared' ? 'この問題集の利用者に反映されます。' : 'この端末だけに保存されます。'}`;
      dialog.returnValue = '';
